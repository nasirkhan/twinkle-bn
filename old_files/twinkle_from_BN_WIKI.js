...
Twinkle.shared.callback = function friendlysharedCallback( uid ) {
	...
	form.append( { type:'header', label:'শেয়ার আইপি অ্যাড্রেস টেমপ্লেট' } );
	...
	var org = form.append( { type:'field', label:'হোস্ট নাম,যোগাযোগের তথ্য ও আইপি অ্যাড্রেস মালিক/অপারেটর পূরণ করুন ( যদি প্রয়োগযোগ্য হয়) ও  \"দাখিল\" করুন' } );
	org.append( {
			type: 'input',
			name: 'organization',
			label: 'সংস্থার নাম',
	...
	org.append( {
			type: 'input',
			name: 'contact',
			label: 'যোগাযোগের তথ্য ( কেবলমাত্র যদি অনুরোধ করা হয়ে থাকে)',
			disabled: true,
...
Twinkle.welcome.normal = function() {
	...
	if( mw.config.get( 'wgNamespaceNumber' ) === 3 ) {
		var username = mw.config.get( 'wgTitle' ).split( '/' )[0].replace( /\"/, "\\\""); // only first part before any slashes
		$(twAddPortletLink("#", "স্বাগতম", "friendly-welcome", "ব্যবহারকারিকে স্বাগতম", "")).click(function() { Twinkle.welcome.callback(username); });
	}
};
...
Twinkle.welcome.welcomeUser = function welcomeUser() {
	...
	Wikipedia.actionCompleted.redirect = mw.config.get('wgPageName');
	Wikipedia.actionCompleted.notice = "স্বাগতম বার্তা পাঠানো হয়েছে,আলাপ পাতা কিছুক্ষনের মধ্যেই ফিরিয়ে আনা হচ্ছে।";
...
Twinkle.welcome.callback = function friendlywelcomeCallback( uid ) {
	if( uid === mw.config.get('wgUserName') ){
		alert( 'আপনাকে স্বাগতম ! আপনাকে  সাগ্রহে স্বাগতম !' );
		return;
	}
	
	var Window = new SimpleWindow( 600, 400 );
	Window.setTitle( "ব্যবহারকারিকে স্বাগতম" );
	Window.setScriptName( "টুইংকল" );
...
Twinkle.closer.actions = {
	afc: function twinklecloserActionsAfc( section ) {
		...
		Window.addFooterLink( "টুইংকল সাহায্য", "WP:TW/DOC#closer-afc" );
	...
	afd: function twinklecloserActionsAfd( section, page ) {
		...
		Window.addFooterLink( "টুইংকল সাহায্য", "WP:TW/DOC#closer-afd" );
...

{
	...
	preferences: [
		...
		// If to offer a prompt for extra summary reason for normal reverts, default to true
		{
			...
			label: "Prompt for reason for normal রোলব্যাকs",
			helptip: "\"Normal\" রোলব্যাকs are the ones that are invoked from the middle [rollback] link.",
			type: "boolean"
		},
		...
		// Note from TTO: |contribs| seems to be equal to |others| + |mine|, i.e. redundant, so I left it out heres
		{
			...
			label: "Show রোলব্যাক links on these pages",
			type: "set",
...
;(function(){
	...
	callback = function() {
		...
		Window.addFooterLink( "টুইংকল সাহায্য", "WP:TW/DOC#deprod" );
...
Twinkle.fluff = {
	auto: function() {
		if( parseInt( QueryString.get('oldid'), 10) !== mw.config.get('wgCurRevisionId') ) {
			// not latest revision
			alert("Can't রোলব্যাক, page has changed in the meantime.");
			return;
		}
...
Twinkle.image.callback = function twinkleimageCallback() {
	...
	Window.setTitle( "দ্রুত অপসারণের জন্য ফাইল" );
	Window.setScriptName( "টুইংকল" );
...
Twinkle.imagetraverse.callback = function() {
	...
	Window.setScriptName( "টুইংকল" );
	Window.addFooterLink( "টুইংকল সাহায্য", "WP:TW/DOC#imagetraverse" );
...
Twinkle.protect = function twinkleprotect() {
	...
	if ( userIsInGroup( 'sysop' ) ) {
		$(twAddPortletLink("#", "পাতা সুরক্ষা", "tw-rpp", "পাতা সুরক্ষা করুন", "")).click(Twinkle.protect.callback);
	} else if (twinkleUserAuthorized) {
		$(twAddPortletLink("#", "সুরক্ষা আবেদন", "tw-rpp", "পাতাটি সুরক্ষা করার জন্য আবেদন করুন", "")).click(Twinkle.protect.callback);
	} else {
		$(twAddPortletLink("#", 'সুরক্ষা আবেদন', 'tw-rpp', 'পাতাটি সুরক্ষা করার জন্য আবেদন করুন', '')).click(function() {
			alert("নবাগত,আপনার অ্যাকাঊন্টটি  টুইংকল ব্যবহার করার জন্য নয়।");
		});
	}
};
...
Twinkle.xfd = function twinklexfd() {
	...
	if (twinkleUserAuthorized) {
		$(twAddPortletLink("#", "এক্সএফডি", "tw-xfd", "(এক্সএফডি)অন্য সকল অপসারণের প্রস্তাবনা", "")).click(Twinkle.xfd.callback);
	} else {
		$(twAddPortletLink("#", 'এক্সএফডি', 'tw-xfd', '(এক্সএফডি)অন্য সকল অপসারণের প্রস্তাবনা', '')).click(function() {
			alert("নবাগত,আপনার অ্যাকাঊন্টটি  টুইংকল ব্যবহার করার জন্য নয়।");
		});
	}
};
